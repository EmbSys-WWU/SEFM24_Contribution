...
public class SCPointer extends SCVariable {
	...
	public SCPointer(String name, String type, boolean stat, boolean cons,
			List<String> otherMods, Expression init) {
		super(name);
		this.type = type;
		this._static = stat;
		this._const = cons;
	...
	public String getDeclarationString() {
		return (_static ? "static " : "") + (_const ? "const " : "")
				+ this.type + "* " + this.name;
	}

	/**
	 * initiates a new SCPointer with a different name but the same internal state EXCEPT:
	 * all linked objects will be set to null to avoid changes by accident
	 * @param newName new name of the variable
