...
public abstract class AbstractNodeTransformer implements NodeTransformer {
    ...
    /**
     * Finds a child node of the given node that has the given node name. Returns null if it cannot be
     * found.
     * 
     * @param node
     * @param name
    ...
    /**
     * (Recusively) Finds a child node of the given node that has the given node name. Returns null if
     * it cannot be found.
     * 
     * @param node
     * @param name
    ...
    /**
     * Finds the first real child node that has NOT the given name. A real child node is one that is not
     * just a text entry. More specifically, it is one surrounded by XML tags.
     * 
     * @param node
     * @param name
     * @return
    ...
    protected Node findFirstChildNot(Node node, String name) {
        ...
        for (int i = 0; i < nodes.getLength(); i++) {
            Node child = nodes.item(i);
            if (!child.getNodeName().equals(name) && !child.getNodeName().equals("#text")) {
                return child;
            }
        }
        return null;
    ...
    /**
     * Finds all child nodes with the given name and returns them as a list. If no one can be found, it
    ...
    /**
     * (Recursively) Finds all child nodes with the given name and returns them as a list. If no one can
     * be found, it returns an empty list of nodes.
     * 
     * @param node
     * @param name
     * @param useRecursion
    ...
    /**
     * Finds all real child nodes of the given node. Real nodes are those that are not text entry nodes,
     * and which are surrounded by XML tags.
     * 
     * @param node
     * @return
     */
    ...
    /**
     * Finds the first real child node that has the given name. A real child node is one that is not
     * just a text entry. More specifically, it is one surrounded by XML tags.
     * 
     * @param node
     * @param name
    ...
    /**
     * Obtains a transformer for the given node from the transformer factory and runs its transformation
     * method.
     * 
     * @param node
     * @param environment
    ...
    /**
     * Creates a prefix string out of the prefix stack by popping one value after the other from the
     * stack and attaching it to the front of the string to be returned. All values are separated by the
     ...
     * 
     * if(stack == null) { return prefix; } for(int i = 0; i < stack.size(); i++) { prefix +=
     * stack.get(i) + Constants.PREFIX_DELIMITER; }
     * 
     * return prefix; }
     */
