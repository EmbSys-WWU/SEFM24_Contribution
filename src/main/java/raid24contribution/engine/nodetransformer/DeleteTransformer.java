...
public class DeleteTransformer extends AbstractNodeTransformer {
	private static Logger logger = LogManager.getLogger(DeleteTransformer.class
			.getName());
	...
	public void transformNode(Node node, Environment e) {
		...
		if (currStacksize == prevStacksize + 1) {
			...
			logger.error("{}: Could not determine 'delete' expression ",
					NodeUtil.getFixedAttributes(node));
		}
		if (ret != null) {
			e.getExpressionStack().add(ret);
		} else {
			logger.error("{}: Couldn't transform 'delete' expression",
					NodeUtil.getFixedAttributes(node));
		}
	}
}
